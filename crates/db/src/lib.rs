//! In-memory stores for CanisLink alpha control plane.
...
pub struct InviteStore {
    ...
    /// invite timestamps for rate limit (dog -> times)
...
impl PolicyStore {
    ...
    pub fn dog_for_terminal(&self, terminal: TerminalId) -> Option<DogId> {
        self.terminal_dog.lock().get(&terminal).copied()
    }
